...
class ModeloCompra():
    ...
    def listar_compras_usuario(self,db,usuario):
        try:
            cursor=db.connection.cursor()
            sql="""SELECT COM.fecha, LIB.isbn, LIB.titulo, LIB.precio FROM compra COM JOIN libro LIB ON COM.libro_isbn = LIB.isbn where COM.usuario_id={0}""".format(usuario.id)
            cursor.execute(sql)
